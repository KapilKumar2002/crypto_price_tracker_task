...
final realTimePriceProvider =
    StreamProvider.family<double, String>((ref, symbol) {
  final repository = ref.watch(coinRepositoryProvider);
  return repository.getRealTimePrice(symbol);
});

final historicalDataProvider =
    FutureProvider.family<List<double>, Map<String, String>>(
        (ref, params) async {
  final repository = ref.watch(coinRepositoryProvider);
  return repository.getHistoricalData(params['symbol']!, params['interval']!);
});
